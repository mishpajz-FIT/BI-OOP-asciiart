  ...
  def apply(filePath: String): Try[ImageFileImporter] = {
    Try {
      ...
      if (!file.exists() || !file.isFile) {
        throw ImporterException(s"File at path $filePath is not usable file or doesn't exist")
      }

      val fileType = getFileType(file) match {
        case Some(value) => value
        case None => throw ImporterException(s"File at path $filePath could not be opened or processed")
      }

      if (!allowedFileTypes.contains(fileType)) {
        throw ImporterException(s"File at path $filePath has unsupported file type $fileType")
      }

      try {
        new ImageFileImporter(file)
      } catch {
        case NonFatal(e) => throw ImporterException(s"File at path $filePath could not be processed due to error ${e.getMessage}")
      }
    }
  }
